...
- [ ] **T-250** `GET /api/play/stream?cursor=` SSE route: polls `activity_log` after the
...
- [ ] **T-251** In-world side panel for the selected task: status select (allowed
...
- [ ] **T-252** Pending handoffs render as a courier (character with parcel) walking
...
- [ ] **T-253** Lobby boards per visible event: name, countdown (same component as the
...
- [ ] **T-254** On enter, camera focuses the actor's desk and a "Today" tray lists: due
...
- [ ] **T-256** `prefers-reduced-motion`: no walking/particles, objects snap, camera cuts;
